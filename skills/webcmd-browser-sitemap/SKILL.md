...
3. Prefer the workflow's **Best path**. If it names an adapter such as `webcmd twitter post`, use that before raw browser actions.
...
```text
~/.webcmd/sites/<site>/sitemap/    # local overlay
...
1. Find the local workflow file under `~/.webcmd/sites/<site>/sitemap/workflows/` whose `Best path` references the adapter command.
